...
class MF1AuthArgsUnit(DeviceRequiredUnit):
    ...
    def get_param(self, args):
        class Param:
            def __init__(self):
                self.block = args.blk
                self.type = MfcKeyType.KEY_B if args.b else MfcKeyType.KEY_A
                key: str = args.key
        ...
        try:
            if args.port is None:  # PN532 auto-detect if no port is supplied
                ...
                if "Microsoft" in platform_name:
                    ...
                    shell_path = None
                    shell_cmd = PathManager.get_shell_command()
                    for prefix in path:
                        fn = os.path.join(prefix, shell_cmd)
...
class HfMfRdbl(MF1AuthArgsUnit):
    def on_exec(self, args: argparse.Namespace):
        block = args.blk
        key_type = MfcKeyType.KEY_B if args.b else MfcKeyType.KEY_A
        key: str = args.key
        if not re.match(r"^[a-fA-F0-9]{12}$", key):
            raise ArgsParserError("key must include 12 HEX symbols")
        ...
        if resp[0]["sak"] in block_size_dict:
            ...
            if self.cmd.isGen1a():
                ...
                for block in range(64):
                    ...
                    if(block == 0):
                        if len(resp[0]["uid"]) == 7:
                            blockData = "041219c3219316984200e32000000000"
                        else:
                            blockData = "11223344440804001122334455667788"
                    elif(is_trailer_block(block)):
