...
from pn532_enum import (Command, Pn532KillerCommand, MifareCommand, TagFile, Status,
                     SlotNumber, TagSenseType, MfcKeyType, PN532KillerMode,
                     PN532KillerTagType, PN532KillerSnifferMode)
...
class Pn532Com:
    ...
    def open(self, port, baud=None) -> "Pn532Com":
        if not self.isOpen():
            error = None
            try:
                ...
                if baud and hasattr(self.communication, 'set_baudrate'):
                    self.communication.set_baudrate(baud)
    ...
    def close(self):
        ...
        self.wait_response_map.clear()
        self.send_data_queue.queue.clear()
        self.ignore_late_cmd_until.clear()

    def set_normal_mode(self) -> "Response":
        self.communication.write(bytes.fromhex("5500000000000000000000000000"))
        resp = self.send_cmd_sync(Command.SAMConfiguration, bytes.fromhex("01"))
        return resp

    def in_release(self) -> "Response":
        resp = self.send_cmd_sync(Command.InRelease, bytes.fromhex("00"))
        return resp
    ...
    # PN532Killer
    def set_work_mode(self, mode: PN532KillerMode = PN532KillerMode.READER, type=PN532KillerTagType.MFC, index=0, sniffer_mode: PN532KillerSnifferMode = PN532KillerSnifferMode.WITHOUT_TAG) -> "Response":
        """
        设置 PN532Killer 工作模式
        :param mode: 工作模式 (PN532KillerMode)
        :param type: 标签类型 (PN532KillerTagType)
        :param index: 索引值 (0-7)
        :param sniffer_mode: 嗅探模式 (PN532KillerSnifferMode)，仅在 mode 为 SNIFFER 时有效
        :return: 响应对象
        """
        if not isinstance(mode, PN532KillerMode):
            raise ValueError("mode 必须是 PN532KillerMode 类型")
            
        if isinstance(type, int):
            type_value = type
        else:
            if not isinstance(type, PN532KillerTagType):
                raise ValueError("type 必须是 PN532KillerTagType 类型或整数")
            type_value = type.value
            
        if not 0 <= index <= 7:
            raise ValueError("index 必须在 0-7 范围内")
            
        # 嗅探模式只支持 Mifare Classic
        if mode == PN532KillerMode.SNIFFER and type_value != PN532KillerTagType.MFC.value:
            raise ValueError("嗅探模式只支持 Mifare Classic 卡片类型")
            
        # 如果是嗅探模式，使用 sniffer_mode 参数
        if mode == PN532KillerMode.SNIFFER:
            if not isinstance(sniffer_mode, PN532KillerSnifferMode):
                raise ValueError("sniffer_mode 必须是 PN532KillerSnifferMode 类型")
            index = sniffer_mode.value
            
        response = self.send_cmd_sync(
            Pn532KillerCommand.SetWorkMode,
            [mode.value, type_value, index],
            timeout=2  # 增加超时时间以确保命令有足够时间执行
        )
        return response

    def read_sniffer_data(self, mode: PN532KillerSnifferMode = PN532KillerSnifferMode.WITHOUT_TAG, timeout: int = 2) -> "Response":
        """
        读取嗅探数据
        :param mode: PN532KillerSnifferMode，无标签/带标签
        :param timeout: 超时时间
        :return: Response，parsed 包含 {'mode': PN532KillerSnifferMode, 'records': [bytes,...], 'tail': bytes}
        """
        if not isinstance(mode, PN532KillerSnifferMode):
            raise ValueError("mode 必须是 PN532KillerSnifferMode 类型")

        # 按照提供的示例帧格式构造数据：D4 20 01 <index>
        index = 0 if mode == PN532KillerSnifferMode.WITHOUT_TAG else 1
        data = bytes([0x01, index])
        resp = self.send_cmd_sync(Pn532KillerCommand.ReadSniffData, data, timeout=timeout)

        # 解析响应：数据格式为 [mode_flag][records...][tail]
        # 无标签模式每条记录20字节；有标签模式每条记录24字节。
        if resp.status == Status.SUCCESS and len(resp.data) >= 1:
            try:
                # 根本性修正：固件返回的是原始数据流，而非固定长度的数据块。
                # 此前的切割逻辑是错误的，导致了数据污染。
                # 正确做法是：将第一个字节作为模式标志，剩余的全部作为单一的、连续的数据记录。
                mode_flag = resp.data[0]
                payload = resp.data[1:]
                resp.parsed = {
                    'mode': PN532KillerSnifferMode.WITHOUT_TAG if mode_flag == 0 else PN532KillerSnifferMode.WITH_TAG,
                    'records': [payload],  # 将整个 payload 视为单一记录
                    'tail': b'',
                }
            except Exception:
                # 解析失败时保留原始数据
    ...
    def parse_sniffer_auth_data(self, all_data: bytes, sniff_mode: PN532KillerSnifferMode):
        """
        从嗅探器原始数据中解析 MIFARE Classic 认证数据。
        根据嗅探模式（有标签/无标签）采用不同策略。

        此方法现在调用独立的 SnifferParser 模块。
        """
        from sniffer_parser import SnifferParser
        return SnifferParser.parse_sniffer_auth_data(all_data, sniff_mode)

    def clear_sniffer_log(self, timeout: int = 2) -> "Response":
        """
        清除嗅探日志 (Pn532Killer 私有命令 0x22)。

        长期嗅探时固件缓冲(无标签 160B / 带标签 96B)会循环覆盖旧数据。
        在多次嗅探会话之间调用此命令,确保读到的是本次会话的数据。

        命令格式: D4 22 01 00
        """
        return self.send_cmd_sync(
            Pn532KillerCommand.ClearSnifferLog,
            bytes([0x01, 0x00]),
            timeout=timeout,
        )
